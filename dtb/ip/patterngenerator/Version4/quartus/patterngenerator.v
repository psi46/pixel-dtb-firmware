...
(
  ...
  */
	...
	// --- clock crossing logic for control signals --------------------------
	cdc_state cdc_enable (csi_clock_clk, clk, clkena, csi_clock_reset, ctrl_ena,   enable);
	cdc_state cdc_ext    (csi_clock_clk, clk, clkena, csi_clock_reset, ctrl_ext,   ena_ext);
	cdc_state cdc_loop   (csi_clock_clk, clk, clkena, csi_clock_reset, ctrl_loop,  ena_loop);
	cdc_pulse cdc_start  (csi_clock_clk, clk, clkena, csi_clock_reset, ctrl_start, set_start);

	
	// --- trigger generator --------------------------------------------------
	wire trig_loop = loopcnt == 0;
  
	always @(posedge clk or posedge csi_clock_reset)
	begin
