...
set_max_skew \
  ...
  -to "*|deser400_PSI:ds400_I|cdr:cdr1|oversampling:ovs|s1[*]"   0.1 \
  -exclude { from_clock to_clock clock_uncertainty }
  
set_max_skew \
  -from [get_ports LVDS2LCDS_sdata_II] \
  -to "*|deser400_PSI:ds400_II|cdr:cdr1|oversampling:ovs|s1[*]"  0.1 \
  -exclude { from_clock to_clock clock_uncertainty }
  
set_max_skew \
  -from [get_ports LVDS2LCDS_sdata_III] \
  -to "*|deser400_PSI:ds400_III|cdr:cdr1|oversampling:ovs|s1[*]" 0.1 \
  -exclude { from_clock to_clock clock_uncertainty }
  
set_max_skew \
  -from [get_ports LVDS2LCDS_sdata_IV] \
  -to "*|deser400_PSI:ds400_IV|cdr:cdr1|oversampling:ovs|s1[*]"  0.1 \
  -exclude { from_clock to_clock clock_uncertainty }
